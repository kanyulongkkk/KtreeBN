...
static SEXP ast_discrete(SEXP xx, SEXP yy, SEXP zz, SEXP ff, SEXP x, SEXP y,
    ...
    test_e test, double a, int debuglevel, int test_epi) {
...
SEXP retval;
  ...
  if (test_epi == 3)
  {
    for (cursize = fmax(2, minsize); cursize <= maxsize; cursize++)
    {
...
SEXP allsubs_test(SEXP x, SEXP y, SEXP sx, SEXP fixed, SEXP data, SEXP test,
    SEXP B, SEXP alpha, SEXP min, SEXP max, SEXP debug, SEXP epi) {

int minsize = INT(min), maxsize = INT(max), debuglevel = isTRUE(debug), nobs = 0, test_epi = INT(epi);
int nsx = length(sx), nf = length(fixed);
double a = NUM(alpha);
...
SEXP xx, yy, zz, ff = R_NilValue, res = R_NilValue;
  ...
  if (IS_DISCRETE_ASYMPTOTIC_TEST(test_type)) {
    ...
    res = ast_discrete(xx, yy, zz, ff, x, y, sx, fixed, nobs, nsx, nf, minsize,
            maxsize, test_type, a, debuglevel, test_epi);
