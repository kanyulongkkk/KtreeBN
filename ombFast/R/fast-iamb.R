...
fast.ia.markov.blanket = function(x, data, nodes, alpha, B, whitelist, blacklist,
  ...
  if (fast.epi == 2)
  {
    # we consider Class first
    if (x == names(data)[length(data)] & length(mb) == 0) {
      
      temp_mb = character(0)
      
      copy_nodes = nodes
      
      for (temp_node in copy_nodes) {
        
        mb = temp_node
        
        nodes = copy_nodes[copy_nodes %!in% mb]
        ...
        repeat {
          ...
          if (debug) {
            ...
            cat("  * assuming that", temp_node, "has already included in the markov blanket.\n")
          ...
          else nodes = nodes[nodes %!in% mb]
          
        }#REPEAT
        
        # we have assumed that one node has existed in markov blanket, so there at least two nodes in markov blanket,
        # if only one node left, we have to reset the markov blanket
        if (length(mb) == 1) mb = character(0)
        
        if (debug) 
          cat("  >", x, "markov blanket are '", mb, "'\n")
        
        temp_mb = unique(c(temp_mb, mb))
        
      }#FOR
      
      if (debug) 
        cat("  > The final", x, "markov blanket are '", temp_mb, "'\n")
      
      mb = temp_mb
      
    }#THEN
    
    # now we consider other nodes
    else {
      
      temp_node = names(data)[length(data)]
      mb = c(mb, temp_node)
  ...
  if (fast.epi == 3) 
  {
    ...
    # now we consider the Class node first
    if (x == names(data)[length(data)] & length(mb) == 0) {
      
      # creat a list used to save mb nodes
      temp_mb = character(0)
      
      # make a copy of nodes
      copy_nodes = nodes
      
      len_node = length(nodes)
      
      for (i in 1:(len_node - 1)) {
        
        first_node = copy_nodes[i]
        
        for (j in (i + 1):len_node) {
          
          sec_node = copy_nodes[j]
          
          mb = c(first_node, sec_node)
          ...
          repeat {
            ...
            if (debug) {
              ...
              cat("  * assuming that", first_node, "and", sec_node, "have already included in the markov blanket.\n")
              cat("  * checking nodes for association.\n")
              sapply(names(association),
                     function(x) {  cat("    >", x, "has p-value", association[x], ".\n")})
            ...
            for (node in names(association)) {
              ...
              if ((test %!in% asymptotic.tests) || (opc >= 5)) {
                ...
                mb = unique(c(mb, first_node, sec_node))
            ...
            if (first_node %!in% mb & sec_node %!in% mb) {
      ...
      if (debug) 
        cat("  > the final markov blanket is '", temp_mb, "'.\n")
    ...
    # then we consider other nodes
      ...
      class_node = names(data)[length(data)]
      
      for (temp_node in copy_nodes) {
        
        if (temp_node == names(data)[length(data)]) break
        
        mb = c(temp_node, class_node)
        ...
        repeat {
          ...
          if (debug) {
            ...
            cat("  * assuming that", temp_node, "and Class have already included in the markov blanket.\n")
            cat("  * checking nodes for association.\n")
            sapply(names(association),
                   function(x) {  cat("    >", x, "has p-value", association[x], ".\n")})
