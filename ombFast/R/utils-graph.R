...
mb2arcs = function(mb, nodes, debug = FALSE) {

  empty.mb = sapply(mb, function(x) {(length(x$nbr) == 0) || is.null(x$nbr) || identical(x$nbr, "")})
  newnodes <- nodes[!empty.mb]
  result <-  matrix(character(0), ncol = 2, dimnames = list(c(), c("from", "to")))
  tempresult <-  matrix(character(0), ncol = 2, dimnames = list(c(), c("from", "to")))
  ...
  for (x in newnodes) {
    ...
    if (debug) {
      cat("------------------------------------------------\n")
      cat("  >", x, "neighbour is : '", mbnodes, "'.\n")
    }

    for (y in 1:length(mb[[x]][['nbr']])) {
      
      if (debug) 
        cat("  > now considering the neighbourhood '", mbnodes[y], "'.\n")
 
      tempresult <- set.arc.direction(x, mbnodes[y], tempresult)
      
      if (!is.acyclic(tempresult, nodes, directed = FALSE)) {
        
        tempresult <- result
        if (debug) 
          cat("  > introducing cycle, so abandon this arc .\n")
        next
        
      }#THEN
      if (debug) 
        cat("  @ add arc from", x, "to", mbnodes[y], "\n")
      result <- tempresult
